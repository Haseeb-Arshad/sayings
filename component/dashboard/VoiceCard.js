    ...
    <div className="relative z-10 text-gray-400 flex flex-col items-center">
      <div className="w-32 h-40 bg-gray-300 rounded-full mb-4 shadow-inner flex items-center justify-center">
        <div className="flex gap-8">
          <div className="w-4 h-4 bg-black rounded-full opacity-50" />
          <div className="w-4 h-4 bg-black rounded-full opacity-50" />
        </div>
      </div>
...
const VoiceCard = ({ onNewPost }) => {
  ...
  return (
    <div className="col-span-2 row-span-2 bg-surface rounded-3xl p-6 shadow-sm border border-border relative overflow-hidden flex flex-col justify-between">
        ...
        <div className="flex justify-between items-start mb-4">
          ...
          <div
            className={`h-8 w-8 rounded-full flex items-center justify-center transition-colors ${
              recording ? 'bg-red-100 text-red-600' : 'bg-gray-100 text-gray-400'
            }`}
          >
            {recording ? (
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className={`p-3 rounded-full transition-all shadow-sm ${
              recording ? 'bg-red-500 text-white' : 'bg-black text-white hover:bg-gray-800'
            }`}
          >
      ...
      {recording && (
        <div className="absolute inset-0 bg-blue-50/50 pointer-events-none animate-pulse" />
      )}

      {/* audioLevel currently kept for future visualizations */}
      <div className="sr-only">Audio level: {Math.round(audioLevel)}</div>
    </div>
  );
};
